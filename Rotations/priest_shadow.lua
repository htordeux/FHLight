...
local fnOrbs = function(unit)
	...
	if Orbs == 2 and jps.hp(unit) < 0.20 then return true end
	if Orbs == 2 and jps.EnemyHealer[unitguid] then return true end
	if Orbs == 1 and EnemyCaster(unit) == "cac" then return true end
	if Orbs == 1 and EnemyCaster(unit) == "caster" then return true end
	return false
end
...
local parseAggro = {
	...
	{ 112833, jps.IsSpellKnown(112833) and playerhealthpct < 0.85 , "player" , "Aggro_Spectral_" },
}
