...
-- Player Specific
	...
	local playerIsStun = jps.StunEvents()
...
-- Druid
	local function parse_vsDruid()
		...
		{ {"macro","/cast every man for himself\n/cast pummel"}, melee and playerIsStun and jps.isCastingSpell("healing touch",rangedTarget) and jps.CastTimeLeft("target") <=1 , rangedTarget },
		{ {"macro","/cast every man for himself\n/cast pummel"}, melee and playerIsStun and jps.isCastingSpell("cyclone",rangedTarget) and jps.CastTimeLeft("target") <=1 , rangedTarget },
...
-- Paladin
	local function parse_vsPaladin()
		...
		{ "shattering throw", dmgBlock , rangedTarget },
		-- { trinket and pummel after playerIsStun },
		}
		return table
	end
-- Priest
